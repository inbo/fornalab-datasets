...
title <- "Vegetation surveys along edge-to-core transects in open and dense forests in the framework of the Forest Microclimate Assessment (FORMICA) project"
description <- c(
...
data_plot <-
  raw_data_plot %>%
  dplyr::mutate(
    Code = paste0(.data$Region, .data$Elevation, .data$Type, .data$Plot),
...
releve <- 
  ...
  dplyr::mutate(
    id = .data$Code,
    poject = "FORMICA (Forest Microclimate Assessment)",
...
# Set contacts
eml$dataset$contact <- create_eml_contributors
...
eml$dataset$coverage$taxonomicCoverage$taxonomicDescription <- taxonomic_description

# Bibliographic citations
eml$additionalMetadata$metadata$gbif$bibliography$bibliography$citation <- references
```
