...
references <- list(
    list(
    "identifier" = "doi.org/10.1111/jvs.12844",
    "Govaert S, Meeussen C, Vanneste T, et al. Edge influence on understorey plant communities depends on forest management. J Veg Sci. 2020;00:1–12"
    )
  )
```
...
data_herb <- 
  ...
  dplyr::mutate(
    event_id = paste("FORMICA_VEG", "plot", .data$Code, sep = ":"),
    parent_event_id = paste("FORMICA_VEG", "transect", substr(.data$Code, 1, 6), sep = ":"),
    occurrence_id = paste0("UGENT:FORMICA:", .data$Code, ":HERB:", sub(" ", "_", .data$Species_name_long))
  ) %>% 
