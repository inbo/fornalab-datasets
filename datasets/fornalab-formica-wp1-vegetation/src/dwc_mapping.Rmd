...
raw_data_digitalization <- read_excel(here::here(directory, "raw", "Vegetation survey_version36.xlsx"), sheet = "Digitalization info")
```
# Provide metadata

```{r basic metadata}
shortname <- "FORMICA_VEG"
...
description <- c(
  "This is a vegetation survey dataset of ancient deciduous broadleaved forests in temperate Europe. It contains plot data of the herb layer collected in the framework of the Forest Microclimate Assessment (<a href=\"https://formica.ugent.be/\">FORMICA</a>) project.  In total, 125 vegetation relevées of 3x3 m² were surveyed across four gradients:",
...
taxonomic_description <- "All vascular plants (including woody species that are smaller than 1 m."
references <- c("Govaert S, Meeussen C, Vanneste T, et al. Edge influence on understorey plant communities depends on forest management. J Veg Sci. 2020;00:1–12. doi.org/10.1111/jvs.12844")
```
...
surveyors <- 
  data_plot %>% 
  dplyr::select(event_id, Surveyors)
data_herb <- 
  raw_data_herb %>% 
  dplyr::mutate(
    event_id = paste("FORMICA_VEG", "plot", .data$Code, sep = ":"),
    parent_event_id = paste("FORMICA_VEG", "transect", substr(.data$Code, 1, 6), sep = ":"),
    occurrence_id = paste0("UGENT:FORMICA:", .data$Code, ":HERB:", .data$Species_name_long)
  ) %>% 
  dplyr::left_join(surveyors)
coordinates <-  
  data_herb %>% 
...
occurrence <-
  ...
  mutate(
    ...
    recorderdedBy = .data$Surveyors,
    organismQuantity = .data$Cover,
    organismQuantityType = "% coverage of herb layer",
...
eml$dataset$coverage$taxonomicCoverage$taxonomicDescription <- taxonomic_description # does not work

# Bibliographic citations
eml$additionalMetadata$metadata$gbif$bibliography$bibliography$citation <- references # does not work
```
