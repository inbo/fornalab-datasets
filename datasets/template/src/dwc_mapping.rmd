...
subtitle: "Template for vegetation survey data from FORMICA WP1"
...
# Set options for code chunk output appearance
...
```{r message = FALSE}
# Load required R libraries for data import, manipulation, and metadata processing
library(readxl)
library(dplyr)
library(here)
library(digest)        
...
The source data is available in multiple tabs of an Excel file. We need to import each tab separately and in a later step join them together so the data is ready to be mapped to Darwin Core.
Ideally, data is imported from csv and not from Excel files, because Excel is prone to encoding issues.

```{r}
# Define the directory containing the data files
directory <- here::here("datasets", "template", "data")

# Import different sheets for each vegetation stratum, plot characteristics, and survey info
...
We need to provide some basic metadata about the dataset. DatasetID is the DOI of the dataset. A DOI is created once the dataset is registered with GBIF. So in our workflow, we keep this empty for the first publication, and fill it in when republishing the dataset after registration.
...
We first need to join the raw data frames. We also need to create some id's because they are not available in the raw data.
The code is already provided for you here, you just need to run the chunk and try to understand what is happening.

Each identifier needs to be unique and stable. There are multiple options to create ID's:
- Create an identifier with a sequential number (e.g. 1, 2, 3, ...)
- Use a UUID (universally unique identifier) generator to create random unique identifiers
- Create an identifier with meaningful information, for example including the plot code.

1. 'occurrenceID' is the unique identifier for each observation of a species in a vegetation plot. We combined meaningful information with a hash that was generated based on the scientific name. We chose a hash instead of the scientific names, because scientific names can change over time (updates or corrections), while the hash will remain stable because it was added to the raw data.
2. `eventID` is the unique identifier for each vegetation plot surveyed. `eventID`'s are created by combining "FORMICA_VEG", "PLOT" and the 'Code' column from the raw data.
3. `parentEventID` are created by combining "FORMICA", "TRANSECT" and the first 6 characters of the 'Code' column from the raw data (which represent country - elevation - forest type combination). You could also choose a different parenEvent level, because there are multiple hierarchical levels possible (e.g., region, transect).
...
data_plot <-
  ...
  dplyr::mutate(
    # Use 'Code" (country - elevation - forest type - distance to edge combination) to create unique event identifiers for plots
    event_id = paste("FORMICA_VEG", "PLOT", .data$Code, sep = ":"),
    # Use Transect (country - elevation - forest type combination) to create parent event identifiers
...
data_herb <-
  ...
  dplyr::mutate(
    ...
    # 'organism_quantity_type' indicates which layer coverage is measured
...
Now it is your turn! Fill in the Darwin Core fields in the event, occurrence and relevé tables.

```{r dwc transformation}
# Create the 'event' table with required Darwin Core fields
event <- 
  data_plot %>% 
  dplyr::mutate(
    .keep = "none",
...
# Create the 'occurrence' table, mapping each observed species to Darwin Core
occurrence <-
  data_veg %>% 
  mutate(
    .keep = "none",
...
# Prepare summary relevé data (cover percentages per plot)
releve <- 
  data_plot %>%
  dplyr::mutate(
    .keep = "none",
    ...
    coverTotalInPercentage = NA_real_,
    coverTreesInPercentage = NA_real_,
    coverShrubsInPercentage	= NA_real_,
    coverHerbsInPercentage	= NA_real_,
    coverMossesInPercentage	= NA_real_,
    coverLitterInPercentage = NA_real_,
    mossesIdentified = NA,
    lichensIdentified = NA
  )
```
...
# Pick the metadata provider and main contact for this dataset from the creators
...
references <- list(
  list("Govaert S, Meeussen C, Vanneste T, et al. (2019). Edge influence on understorey plant communities depends on forest management. Journal of Vegetation Science. 2020;31:281–292. doi.org/10.1111/jvs.12844")
  )
```
...
logo <- "https://raw.githubusercontent.com/inbo/fornalab-datasets/main/man/figures/logo-fornalab.jpg"
```


# Create metadata file (eml.xml)

The GBIF Metadata Profile is based on the Ecological Metadata Language (EML). Below we provide the required metadata fields. All code is completed, you just need to run the chunks and look at the output. 

## Create metadata file (eml.xml)

### Prepare metadata terms

This piece of code does not require any input, it uses the provided metadata.
...
taxonomic_coverage <-
  list(
        ...
        generalTaxonomicCoverage = taxonomic_description
      )
```

### Function to create contributors in EML format

This function formats each contributor entry in EML structure, including ORCID if present.
...
```{r EML}
# Construct the metadata file using all previous metadata and formatted contributors
eml <- list(
  ...
  dataset = list(
    ...
    coverage = list(
      ...
      temporalCoverage = list(
        rangeOfDates = list(
          beginDate = list(calendarDate = min(event$eventDate)),
          endDate = list(calendarDate = max(event$eventDate))
        )
      ),
...
Check if columns across the tables have the expected data types, e.g., verify event dates and numeric columns.
...
## Check if species names match with the GBIF backbone taxonomic names
...
# Identify unmatched names for error checking
...
'meta.xml' is a static file describing the Darwin Core Archive structure with the definitions. It is manually adapted for each dataset, and here it just needs to be copied to the export folder.

```{r copy meta.xml}
  file.copy(
    here::here("datasets", "template", "extdata", "meta.xml"),
    here::here(dir_export, "meta.xml")
  )
```
